...
PDF·DOCX·Markdown·HWP·TXT의 다양한 확장자를 허용하며, 같은 문서를 다시 올리면 새 버전으로 쌓입니다. 변경 사항을 되돌리고 싶다면 이전 버전을 찾아 검색 대상으로 쉽게 롤백할 수 있고, 같은 문서에 내용이 같은 파일을 새 버전으로 올리면 중복이라고 알려 줍니다. 문서마다 누가 볼 수 있는지를 사용자와 테넌트 단위로 정합니다.
...
찾은 대목의 앞뒤 문단을 함께 돌려주는 옵션이 있습니다. 문단 하나만 떼어 보면 맥락을 알 수 없는 경우가 많기 때문입니다. 설계와 평가 과정은 [검색 설계와 평가](docs/SEARCH.md)에 정리했습니다.

### 유실 없는 비동기 인덱싱

업로드 응답은 인덱싱을 기다리지 않습니다. 대신 문서를 저장하는 트랜잭션과 **같은 원자 단위로** 인덱싱 요청 이벤트를 남깁니다.
이 이벤트는 DB 트리거가 만듭니다. 애플리케이션이 발행을 빠뜨릴 수 없고, 저장이 롤백되면 이벤트도 함께 사라집니다. 남은 이벤트는 릴레이가 카프카로 옮기고 워커가 받아 처리합니다. 처리에 실패한 작업은 간격을 두고 스스로 재시도하며, 최종 실패하면 UI에서 이를 확인하고 버튼을 통해 직접 재시도를 요청할 수 있습니다.
...
REST API와 같은 권한 검사를 거쳐, 에이전트 역시도 사용자와 동등한 인가를 거치도록 제어합니다. 붙이는 방법은 [MCP 서버 붙이기](#mcp-서버-붙이기)에 있습니다.
