...
- **Reranking**: RRF 상위 후보를 랭킹 전용 모델(Cohere Rerank API)로 재정렬하는 기능도 구현해 검증했으나, 정답률이 오히려 떨어지고 지연시간이 늘었다. 기능을 걷어내지는 않고 기본값만 off로 두었다. 요청마다 `rerank` 파라미터로 켤 수 있다 — REST는 요청 본문 필드, MCP는 `search_documents`의 파라미터다. 5절 결과는 모두 off 기준이다.
...
- **Reranking이 나빠진 원인을 규명하지 못함**: 지표가 떨어지는 것만 확인하고 기본값을 off로 돌렸을 뿐, 왜 떨어지는지는 밝히지 못했다. 후보 풀이 30개로 좁아 재정렬할 여지 자체가 적었을 수도 있고, `rerank-v3.5`가 한국어에 약한 것일 수도 있다. 한국어 전용 리랭커(ko-reranker 등)로 바꾸거나 풀 크기를 늘려 재검증할 여지가 있다.
- **MCP 툴 설명이 평가 결과와 어긋남**: `search_documents`의 `rerank` 파라미터 설명은 "정확도가 중요한 요청이면 true로 설정한다"고 안내하는데, 평가에서는 켤수록 정답률이 떨어졌다. 지금 상태로는 사용자가 "꼼꼼하게 찾아줘"라고 할수록 더 나쁜 결과를 받는다. 위 재측정으로 어느 쪽이 맞는지 가린 뒤 툴 설명을 고칠지 파라미터를 내릴지 정해야 한다.
